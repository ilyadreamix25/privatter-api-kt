...
interface UserRepository : JpaRepository<UserEntity, String> {
    @Query("SELECT * FROM user x WHERE x.id = ?1 OR x.auth_key = ?2 OR x.profile_nickname = ?3", nativeQuery = true)
    fun find(id: String? = null, authKey: String? = null, profileNickname: String? = null): UserEntity?

    @Query("SELECT * FROM user x WHERE x.auth_key = ?1", nativeQuery = true)
    fun findByAuthKey(authKey: String): UserEntity?

    @Query("SELECT * FROM user x WHERE x.profile_nickname = ?1", nativeQuery = true)
